...
OUTPUT_THEME_NAME=${OUTPUT_THEME_NAME-oomox-arcana-$THEME}
...
for FILEPATH in "${PATHLIST[@]}"; do
		find "$FILEPATH" -type f -exec sed -i'' \
			...
			-e 's/Name=Arcana/Name=%OUTPUT_THEME_NAME%/g' \
...
rm -fr "${DEST_PATH}"
if [[ "$ARC_TRANSPARENCY" == "false" ]]; then
	mv ./distrib/share/themes/Arcana-Darker-solid "${DEST_PATH}"
	if [[ -d ./distrib/share/themes/Arcana-solid/cinnamon ]] ; then
		mv ./distrib/share/themes/Arcana-solid/cinnamon "${DEST_PATH}"/cinnamon
	fi
	if [[ -d ./distrib/share/themes/Arcana-solid/gnome-shell ]] ; then
		mv ./distrib/share/themes/Arcana-solid/gnome-shell "${DEST_PATH}"/gnome-shell
	fi
else
	mv ./distrib/share/themes/Arcana-Darker "${DEST_PATH}"
	if [[ -d ./distrib/share/themes/Arcana/cinnamon ]] ; then
		mv ./distrib/share/themes/Arcana/cinnamon "${DEST_PATH}"/cinnamon
	fi
	if [[ -d ./distrib/share/themes/Arcana/gnome-shell ]] ; then
		mv ./distrib/share/themes/Arcana/gnome-shell "${DEST_PATH}"/gnome-shell
	fi
fi
